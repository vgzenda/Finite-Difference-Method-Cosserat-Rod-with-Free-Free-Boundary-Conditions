...
def T_SO3(phi):
    """
    Computes the T matrix (Left Jacobian of SO(3)) for the exponential map.
    Relation: (R^T * dR)^v = T(phi) * dphi
    """
    phi = phi.ravel()
    ...
    if norm < 1e-8:
        ...
        term1 = (1 - np.cos(norm)) / (norm**2)
        term2 = (norm - np.sin(norm)) / (norm**3)
        return I + term1 * Phi + term2 * (Phi @ Phi)